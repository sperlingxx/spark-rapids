...
trait GpuExec extends SparkPlan {
  ...
  private lazy val baseMetrics: Map[String, GpuMetric] = {
    ...
    outputRowsLevel.foreach { level =>
      NUM_OUTPUT_ROWS -> createMetric(level, DESCRIPTION_NUM_OUTPUT_ROWS)
    }
    outputBatchesLevel.foreach { level =>
      NUM_OUTPUT_BATCHES -> createMetric(level, DESCRIPTION_NUM_OUTPUT_BATCHES)
    }
    outputDataSizeLevel.foreach { level =>
      OUTPUT_DATA_SIZE -> createSizeMetric(level, DESCRIPTION_OUTPUT_DATA_SIZE)
    }
    b.result()
  }

  protected val outputRowsLevel: Option[MetricsLevel] = Some(DEBUG_LEVEL)
  protected val outputBatchesLevel: Option[MetricsLevel] = Some(DEBUG_LEVEL)
  protected val outputDataSizeLevel: Option[MetricsLevel] = Some(DEBUG_LEVEL)
...
/**
 * GpuExec node that does not produce ColumnarBatch as output.
 */
trait NonColumnarOutputExec extends GpuExec {
