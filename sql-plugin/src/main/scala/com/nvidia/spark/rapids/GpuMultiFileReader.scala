  ...
  def updateMetrics(metrics: Map[String, GpuMetric]): Unit = {
    getColumnStatistics match {
      ...
      case stats =>
        ...
        metrics("maxCPR").set(metrics("maxCPR").value max ((1.0 - minR) * 1000L).toLong)
        if (metrics("minCPR").value == 0L) {
          metrics("minCPR").set(1000L)
        }
        metrics("minCPR").set(metrics("minCPR").value min ((1.0 - maxR) * 1000L).toLong)
